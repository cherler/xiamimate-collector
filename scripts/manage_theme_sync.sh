...
PID_FILE="$LOG_DIR/sync_theme_features_to_pg.pid"
LOG_FILE="$LOG_DIR/sync_theme_features_to_pg.log"
...
PLIST_NAME="com.xiamimate.theme-sync"
...
install_launchd() {
    ...
    if is_launchd_loaded; then
        ...
        echo ""
        echo "管理命令:"
        echo "  状态: bash scripts/manage_theme_sync.sh status"
        echo "  停止: bash scripts/manage_theme_sync.sh stop"
        echo "  启动: bash scripts/manage_theme_sync.sh start"
        echo "  卸载: bash scripts/manage_theme_sync.sh uninstall"
    else
...
start_via_launchd() {
    if [[ -f "$PLIST_DST" ]]; then
        ...
        sleep 1
        echo "theme sync started via launchd"
        return 0
    fi
    return 1
}
...
stop_via_launchd() {
    if is_launchd_loaded; then
        ...
        echo "theme sync stopped via launchd (will auto-restart due to KeepAlive)"
        echo "  若要彻底停止: bash scripts/manage_theme_sync.sh uninstall"
        return 0
    fi
    return 1
}
...
start_sync() {
    ...
    if is_running; then
        echo "theme sync already running: PID $(resolve_pid)"
        print_pg_target
        print_pg_tunnel
        return 0
    fi
    ...
    sleep 1
    if ! is_running; then
        echo "failed to start theme sync; check log: $LOG_FILE"
        cleanup_metadata
        return 1
    fi

    echo "theme sync started: PID $(cat "$PID_FILE")"
...
stop_sync() {
    if ! is_running; then
        cleanup_metadata
        echo "theme sync is not running"
        return 0
    fi
    ...
    if ! wait_for_shutdown "$pid"; then
        echo "theme sync did not stop gracefully; forcing kill: PID $pid"
        kill -9 -- "-$pid" 2>/dev/null || kill -9 "$pid" 2>/dev/null || true
        if ! wait_for_shutdown "$pid" 25 0.2; then
            echo "failed to stop theme sync: PID $pid"
            return 1
        fi
    fi

    cleanup_metadata
    echo "theme sync stopped: PID $pid"
}

status_sync() {
    if is_running; then
        echo "theme sync running: PID $(resolve_pid)"
        echo "log file: $LOG_FILE"
...
case "${1:-}" in
    ...
    restart)
        if is_launchd_loaded; then
            launchctl stop "$PLIST_NAME"
            sleep 2
            echo "theme sync restarted via launchd"
        else
            restart_sync
        fi
        ;;
    status)
        if is_launchd_loaded; then
            echo "theme sync managed by launchd: $PLIST_NAME"
    ...
    preview)
        ...
        ;;
    *)
        echo "Usage: bash scripts/manage_theme_sync.sh {start|stop|restart|status|install|uninstall|logs|preview}"
        echo ""
