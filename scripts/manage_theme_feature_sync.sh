...
start_sync() {
    ...
    sleep 1
    if ! is_running; then
        echo "failed to start theme feature sync; check log: $LOG_FILE"
        cleanup_metadata
        return 1
    fi
...
stop_sync() {
    ...
    cleanup_metadata
    echo "theme feature sync stopped: PID $pid"
}
...
case "${1:-}" in
    ...
    preview)
        ...
        ;;
    *)
        echo "Usage: bash scripts/manage_theme_feature_sync.sh {start|stop|restart|status|logs|preview}"
        exit 1
        ;;
esac